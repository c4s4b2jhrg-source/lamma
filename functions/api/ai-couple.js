export async function onRequestPost(context){
  try{
    ...
    const location=String(incoming.get('location')||'تلقائي — اختَر الأنسب');
    const pose=String(incoming.get('pose')||'تلقائي — عدّل الوقفة');
    const smartFix=String(incoming.get('smartFix')||'1')!=='0';
    const notes=String(incoming.get('notes')||'').trim().slice(0,250);
    const count=Math.max(1,Math.min(4,Number(incoming.get('imageCount')||1)));
    const refs=[];
    for(let i=1;i<=count;i++){const file=incoming.get('image'+i);const role=String(incoming.get('role'+i)||('reference_'+i));if(file instanceof File)refs.push({file,role})}
    ...
    if((occasion==='زواج'||occasion==='ملكة / عقد قران')&&(!roleSet.has('groom_current')||!roleSet.has('bride_current')))return json({error:'ارفع الصورتين الحاليتين أول.'},400);
    const styleHint={
    ...
    const locationHint={
      'تلقائي — اختَر الأنسب':'Choose the most suitable tasteful location for the selected occasion and scene.',
      'استوديو أنيق':'Elegant premium photography studio with clean refined background.',
      'قاعة فخمة':'Luxurious elegant event hall, tasteful Gulf invitation atmosphere.',
      'خارجي بإضاءة طبيعية':'Outdoor setting with flattering soft natural light and uncluttered background.',
      'منزل مرتب ودافئ':'Warm elegant home interior, tidy and tasteful, no clutter.',
      'خلفية بسيطة وناعمة':'Simple soft premium background with minimal distractions.'
    }[location]||location;
    const poseHint={
      'تلقائي — عدّل الوقفة':'Choose and correct the pose automatically so it looks natural, flattering, balanced and believable.',
      'وقفة طبيعية':'Natural standing pose, relaxed shoulders, believable body proportions and hands.',
      'جلوس طبيعي':'Natural seated pose with comfortable posture, realistic hands and body proportions.',
      'لقطة قريبة':'Close portrait framing while keeping the full face clear and undistorted.',
      'ثلاثة أرباع':'Three-quarter portrait angle with the face clearly visible.',
      'لقطة عفوية':'Candid natural-looking pose with believable interaction and expression.'
    }[pose]||pose;
    ...
    const roleDescriptions={groom_current:'current adult groom — unique person A',bride_current:'current adult bride — unique person B',groom_child:'childhood reference of the SAME groom',bride_child:'childhood reference of the SAME bride',graduate_current:'primary current identity reference of the graduate',graduate_secondary:'second angle/reference of the SAME graduate',graduate_third:'third angle/reference of the SAME graduate',birthday_current:'primary current identity reference of the birthday person',birthday_secondary:'second angle/reference of the SAME birthday person',birthday_third:'third angle/reference of the SAME birthday person',parent_primary:'primary parent identity reference',newborn:'newborn identity reference, only if this image is provided',parent_secondary:'second parent identity reference',host_primary:'primary host identity reference',host_secondary:'second person identity reference',primary:'primary identity reference',secondary:'second identity reference'};
    const roleText=refs.map((x,i)=>`Input image ${i+1}: ${roleDescriptions[x.role]||x.role}.`).join('\n');
    const samePersonRefCount=refs.filter(x=>samePersonRoles.has(x.role)).length;const hasNewborn=roleSet.has('newborn');
    const occasionHint={
    ...
    const smartFixText=smartFix?`\nSMART CORRECTION — ENABLED:\n- You may adjust the background/location, pose, body angle, hand placement, crop, camera framing, clothing neatness, lighting, clutter, spacing between people and composition whenever needed to improve the result.\n- Correct awkward anatomy, unnatural posture, extra fingers/hands, bad framing, tangencies, distracting objects and visual inconsistencies.\n- Keep the final result natural rather than over-edited.\n- Never use these corrections as a reason to alter facial identity.`:'';
    const notesText=notes?`\nUSER REQUESTED ADJUSTMENTS:\n${notes}`:'';
    const prompt=`${occasionHint}\n\nLOCATION / BACKGROUND:\n${locationHint}\n\nPOSE / FRAMING:\n${poseHint}${smartFixText}${notesText}\n\nREFERENCE MAP:\n${roleText}\n\nIDENTITY PRESERVATION — HIGHEST PRIORITY:\n- Preserve each person's real facial identity as closely as possible.\n- Keep face shape, eye shape and spacing, eyebrows, nose, lips, jawline, cheeks, skin tone, hairstyle/hairline, apparent age, and distinctive features consistent with the references.\n- Do not beautify, idealize, slim the face, enlarge eyes, reshape nose or lips, change age, change skin tone, or invent a new face.\n- If multiple images belong to the same person, treat them as multiple angles of ONE identity and never generate duplicate people from them.\n- If the scene contains two different people, never mix, blend, swap, or average their faces.\n- Prefer front-facing or three-quarter angles. Avoid extreme profile views if they reduce likeness.\n- If a pose would hide or distort a face, use a gentler version that keeps the face visible.\n- Change pose, clothes, props, scene and background as needed, but facial identity must remain unchanged.\n\nCLOTHING AND SAFETY:\n- Everyone must be fully dressed in neat, modest, age-appropriate clothing.\n- No shirtless or bare-chested person.\n- No revealing clothing.\n- Family-friendly and suitable for a Gulf digital invitation.\n\nVISUAL STYLE:\n- ${styleHint}.\n- Premium invitation-quality composition.\n- Natural skin, believable anatomy, clean hands, coherent lighting and gentle expressions.\n\nSTRICTLY AVOID:\nidentity drift, face redesign, face swapping, blended identities, duplicate people from multiple reference angles, wrong age, beauty-filter look, distorted faces, extra fingers, extra hands, duplicated people, unintended extra people, cropped heads, text, logos, watermarks.`;
    const form=new FormData();form.append('prompt',prompt);refs.forEach((x,i)=>form.append('input_image_'+i,x.file,x.file.name||('reference'+(i+1)+'.jpg')));form.append('width','1024');form.append('height','1024');form.append('guidance','4');
    const serialized=new Response(form);const result=await context.env.AI.run('@cf/black-forest-labs/flux-2-klein-4b',{multipart:{body:serialized.body,contentType:serialized.headers.get('content-type')}});
    let b64='';if(result&&typeof result.image==='string')b64=result.image;else if(result instanceof ReadableStream){const bytes=new Uint8Array(await new Response(result).arrayBuffer());b64=bytesToBase64(bytes)}
    if(!b64)return json({error:'ما رجعت صورة من خدمة الذكاء الاصطناعي.'},502);
    return json({image:b64,provider:'cloudflare',model:'flux-2-klein-4b',occasion,scene,location,pose,smartFix,references:refs.length});
  }catch(e){const msg=String(e&&e.message||'');if(/quota|limit|neuron|billing|exceed/i.test(msg))return json({error:'خلص الحد المجاني لليوم. جرّب مرة ثانية باچر 🤍'},429);return json({error:'صار خطأ أثناء توليد الصورة. جرّب مرة ثانية.'},500)}
}
